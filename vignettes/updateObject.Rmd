...
## Set some fake committer identity:
Sys.setenv(GIT_COMMITTER_NAME="titi", GIT_COMMITTER_EMAIL="titi@gmail.com")
```
...
- By default `updateBiocPackageRepoObjects()` does _not_ try to push the
  ...
  the `r Biocpkg("TimiRGeN")` package can do that. If you are using
  `updateBiocPackageRepoObjects()` on a package that you maintain and
  you wish to push the changes to `git.bioconductor.org`, then do NOT use
  HTTPS access (i.e. don't use `use.https=TRUE`) and use `push=TRUE`.
...
- The `RELEASE_3_13` branch of all Bioconductor packages got frozen in
  ...
  on the development version (i.e. the `master` branch) of a package
